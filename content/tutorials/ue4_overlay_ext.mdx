---
title: 'Tutorial: Building OpenXR Multiapps with UE4'
metaTitle: 'Tutorial: Building OpenXR Multiapps with UE4'
metaDescription: 'Tutorial: Building OpenXR Multiapps with UE4'
---

# Building OpenXR Multiapps with UE4
...
that will with Unreal Engine 4, creating multi apps will be more straightforward than ever and won't require knowledge of coding or any advanced high-skill domain.
...
**SANITY CHECK**: after building, run `hello_xr` (see instructions on the repo) to make sure OpenXR is working properly on your machine. Make sure to run it passing `-g D3D11` as arguments (always - we want to be using DirectX 11).
...
Understand that this part requires creating a new folder and copying select parts of the compiled extension, as well as the loader from the previous steps, as well as some random-seeming ish.
...
- Configurations with known errors:
  ...
  - SteamXR using rift (xrLocateSpace fails for hmd)
