...
2. **deploy** it to Chimera, where it can coexist simultaneously among _other_ XR multi apps. It will also be visible and interactable by other participants in any [Pluto](httsp://pluto.app) conversation.
...
This command will create a new folder bearing your app's name, then enter the folder and install all the dependencies needed for a fresh sandcastle project.
...
- './src/engine _also contains the rest of Sandcastle_, and`./src/examples` contains several examples of utilizing networking, physics, rich media and more. Refer to them and the The Sandcastle [Wiki](https://github.com/PlutoVR/sandcastle/wiki) for more information on individual features.
- `./examples/defaultscene.js` is the scene currently loaded into`engine.js`, and if you open it you'll see the ThreeJS code that is responsible for what you see in your browser. We'll be iterating on this file.
...
Let's animate our cube. As you may know, a ThreeJS is animated the scene by hooking into the browser's built in RequestAnimationFrame() function, which runs many times a second.
...
While without XR Packages we are limited to a _single_ WebXR-scene-as-overlay at a time, it's more than suitable for dev pruposes.
...
Finally, let's turn this app into an [XR Package](../platforms/tb_chimera#xrpackages) so that we can load it alongisde other multi apps as we traverse the immersive web!
...
Load it up in your preferred XR Pacakge environment (we recommend [Chimera](https://chimera.pluto-api.com/) - note that you'll currently have to serve your file remotely using the `Custom-From URL` option), and see it in action alongside other multi apps!

![](./sandcastle/7.gif)

Thanks to the magic of Chimera and Sidecar, if you now start or join a Pluto conversation, other participants (also running Chimera) will be able to see and interact with your app!

We've only scratched the surface, of course - upcoming tutorials will discuss more advanced networking, inputs and more.

That just about wraps up this Sandcastle tutorial; you are now able to jumpstart a WebXR app, iterate on it, turn it into an XR Package and load it in Chimera.

What multi app will you build next?

Feel free to offer suggestions or raise issues to anything you see in this guide on [GitHub](https://github.com/PlutoVR/MAIT/tree/main/content/beginner_tutorials/sandcastle.mdx)!
