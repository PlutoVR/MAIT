...
2. **deploy** it to Pluto's App Launcher, where it can coexist simultaneously among _other_ XR multi apps. It will also be visible and interactable by other participants in any [Pluto](httsp://pluto.app) conversation.
...
Finally, let's turn this app into an [XR Package](../platforms/tb_webxr#xrpackages) so that we can load it alongisde other multi apps as we traverse the immersive web!
...
Load it up in your preferred XR Pacakge environment (we recommend [The Pluto App Launcher](https://chimera.pluto.app/) - note that you'll currently have to serve your file remotely using the `Custom-From URL` option), and see it in action alongside other multi apps!
